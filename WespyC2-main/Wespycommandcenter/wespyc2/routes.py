...
from wespyc2 import app, db
from wespyc2.forms import SignInForm, RegisterForm, TaskForm, StatusForm
from wespyc2.models import User, Tasks, Keys
import hashlib
...
def login():
    if current_user.is_authenticated:
        return redirect(url_for('wespyc2'))
    form = SignInForm()
...
@app.route("/wespyc2", methods=['GET', 'POST'])
@login_required
def wespyc2():
    form = TaskForm()
    if form.validate_on_submit():
    ...
    if recentTasks:
       return render_template("wespyc2.html", form=form, recentTasks=recentTasks)
    else:
        return render_template("wespyc2.html", form=form)
